...
class _HomeState extends State<Home> {
  ...
  void initState() {
    ...
    FirebaseAuth.instance.authStateChanges().listen((user) async{
      ...
      if ( user == null ){
        Navigator.of(context).pop(MaterialPageRoute(builder: (context)=>const LoginScreen()));
        return;
      }

      try{
        Profile? profile = await AuthService.getProfile(user!.email!);
        if (profile == null ) {
          Navigator.of(context).push(MaterialPageRoute(builder: (context)=>const EditProfileScreen(settingProfile: true,)));
          return ;
        }
      }catch(err){

        if (kDebugMode){
          print("failed getting the users profile ----------------------");
          print(err);
        }
      }
                ...
                child: Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      buildAirtimeAndDataWidget(context),
                      const SizedBox(
                        height: 16,
                      ),
                      const AspectRatio(
  ...
  Widget buildAirtimeAndDataWidget(BuildContext context) {
    return Row(
        ...
        Expanded(
          child: AspectRatio(
            ...
            child: SquaredTileButton(
              text: "Purchase Data",
              description: "Buy a data plan for any of the supported networks",
                ...
                showModalBottomSheet(
                    ...
                    builder: (context) {
                      return ServiceProviderSelector(
                        items: NigerianNetworkServiceProviders,
                        onPressed: (provider) async {
                          await Navigator.of(context).push(MaterialPageRoute(
                              builder: (context) => DataPurchaseScreen(
        ...
        Expanded(
          child: AspectRatio(
            ...
            child: SquaredTileButton(
              ...
              description: "Send airtel of the supported supported networks",
              icon: Icons.phone,
              onPressed: () {
                      ...
                      return ServiceProviderSelector(
                        ...
                        onPressed: (provider) {
                          Navigator.of(context).push(MaterialPageRoute(
                              builder: (context) =>
                                  AirtimePurchaseScreen(provider: provider)));
                        },
                      );
                    });
              },
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Row(
            children: [
              Consumer<HomeChangeNotifier>(builder: (context, model, child) {
                return InkWell(
                  onTap: () {
              ...
              children: [
                if (!model.loadingBalance)
                  Row(
                    ...
                    children: [
                      Text(
                        "N${formatAmount(model.balance)}",
                        style: WhiteText.copyWith(
                            fontSize: getBalanceFontSize(model),
                            fontWeight: FontWeight.bold,
                            letterSpacing: 1.2),
                      ),
                      IconButton(
                          onPressed: determineRefreshButtonAction(model),
                          icon: const Icon(
                            Icons.refresh,
                            color: ThemedColor,
                          ))
          ...
          Row(
            children: [
              Expanded(
                  child: TileButton(
                text: "Fund Wallet",
                description: "Add money to your data account",
